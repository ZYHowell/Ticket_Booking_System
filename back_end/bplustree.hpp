...
>   class bplustree{
    ...
    inline pointer* nth_pointer(byte *start, size_t n){
    ...
    */
    inline void left_balance(node &p, node &l, byte *p_cache){
        ...
        *nth_key(left_cache, l.size) = *nth_key(p_cache, 0);
        *nth_pointer(left_cache, l.size) = *nth_pointer(p_cache, 0);
        ++l.size, --p.size;
    ...
    */
    inline void right_balance(node &p, node &r, byte *p_cache){
        ...
        *nth_pointer(right_cache, 0) = *nth_pointer(p_cache, p.size);
    ...
    */
    inline void receive_left(node &p, node &l, byte *p_cache){
        ...
        *nth_key(p_cache, 0)        = *nth_key(left_cache, l.size);
        *nth_pointer(p_cache, 0)    = *nth_pointer(left_cache, l.size);
        ++p.size;--l.size;
        save_cache(left_cache, l);
        save_cache(p_cache, p);
        save_node(l), save_node(p);
    ...
    */
    inline void receive_right(node &p, node &r, byte *p_cache){
        ...
        *nth_pointer(p_cache, p.size)   = *nth_pointer(right_cache, 0);
        ++p.size, --r.size;
        save_cache(*nth_key(right_cache, 1), r);
        save_cache(p_cache, p);
        save_node(r), save_node(p);
    }
    /*
    ...
    */
    void deal_surplus(node &now, node &par, byte *cache, byte *cache_par){
        node tmp;
        if (now.pos != *nth_pointer(cache_par, 0)){
        ...
        if (now.pos != *nth_pointer(cache_par, par.size - 1)){
            ...
            else split(now, cache, par, cache_par);
        }
        else split(now, cache, par, cache_par);
    }
    void deal_deficit(node &now, node &par, byte *cache,byte *cache_par){
        node tmp;
        if (now.pos != *nth_pointer(cache_par, par.size - 1)){
            tmp = load_node(now.next);
    ...
    */
    void split(node &now, byte *cache, node &par, byte *cache_par){
        size_t s = now.size / 2;
        now.size -= s;
        size_t ns = now.size;
        ...
        ns = binary_search_key(cache_par, now.key, par.size);
    ...
    */
    bool _insert(node &now, const key_type &k, const value_type &v, byte *cache = nullptr){
        if (cache == nullptr){
            cache = new byte[inf_size];
            load_cache(cache, now);
        }
        ...
        if (now.type){
            ...
            if (child_node.size >= part_size) {
                deal_surplus(child_node, now, cache_child, cache);
            }
            else save_node(child_node);
            return ret;
        }
        else{
            if (equal(*nth_key(cache, ord), k)) {
                pointer value_pointer = *nth_pointer(cache, ord);
    ...
    //1.5 aims to protect the safty in merge function(an oversized one)
    bool _remove(node &p, const key_type &k, byte *cache = nullptr){
        ...
        if (p.type){
            ...
            if (child_node.size < part_size / 2) {
                deal_deficit(child_node, p, cache);
            }
...
public:
    ...
    bool insert(const key_type &k,const value_type &v){
        if (root.pos == invalid_p){
            ...
            *nth_key(cache, 0) = k;
            *nth_pointer(cache, 0) = alloc_data.alloc(sizeof(key_type) + sizeof(value_type));
            fseek(datafile, *nth_pointer(cache, 0), SEEK_SET);
