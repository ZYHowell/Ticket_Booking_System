...
>   class bplustree{
        ...
        if (now.pos != *nth_pointer(cache_par)){
            ...
            if (tmp.size >= part_size_n / 2){
                ...
                *nth_key_n(cache_par, ord) = now.key;
                ...
                byte cache_tmp[node_size];load_cache_n(cache_tmp, tmp);
        ...
        if (now.pos != *nth_pointer(cache_par)){
            ...
            if (tmp.size >= part_size_l / 2){
                ...
                *nth_key_n(cache_par, ord) = now.key;
                ...
                byte cache_tmp[node_size];load_cache_l(cache_tmp, tmp);
    ...
    */
    ...
    void merge_n(node &now, byte *cache, node &par, byte *cache_par, node &tmp){
        ...
        if (now.size >= part_size_n) split_n(now, cache, par, cache_par);
        save_cache_n(cache, now);
        save_node(now);
    }
    void merge_l(node &now, byte *cache, node &par, byte *cache_par, node &tmp){
        ...
        if (now.size >= part_size_l) split_l(now, cache, par, cache_par);
        save_cache_l(cache, now);
        save_node(now);
    }
    /*
...
public:
    ...
    bool remove(const key_type &k){
        // if (root.pos == invalid_p) return false;
        // bool ret = true;
        // if (root.type){
