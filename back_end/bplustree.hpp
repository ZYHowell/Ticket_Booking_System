...
>   class bplustree{
    ...
    */
    void deal_surplus(node &now, node &par, byte *cache, byte *cache_par, size_t ord){
        node tmp;
    ...
    void deal_deficit(node &now, node &par, byte *cache,byte *cache_par, size_t ord){
        node tmp;
        if (now.pos != *nth_pointer(cache_par, par.size - 1)){
            tmp = load_node(now.next);
