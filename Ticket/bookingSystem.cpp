...
const std::string CMD[numOfCommands] = {
	...
	"clean","exit",
	"login","modify_profile_without_password",
	"display","login_with_info",
	"query_available_ticket","query_my_ticket"
};
...
(const std::string &cmd, const vector<token> &V) {
	...
	case 17: modifyProfile2(V); break;
	case 18: displayAll(); break;
	case 19: login2(V); break;
	case 20: queryQuantity(V); break;
	case 21: queryMyTicket(V); break;
	default: break;
...
void ticketBookingSystem::login(const vector<token> &V) {
	...
	os<<User.login(V[0].second.asint(), V[1].second)<<'\n';
}

void ticketBookingSystem::login2(const vector<token> &V) {
	//for (int i = 0; i < V.size(); i++) std::cout << V[i].second << " "; std::cout << endl;
	os << User.loginWithInfo(V[0].second, V[1].second) << '\n';
}
...
void ticketBookingSystem::modifyProfile2(const vector<token> &V) {
	os << (User.modify_nopswd(V)) << '\n';
}

void ticketBookingSystem::displayAll() {
	Train.displayAll(os);
}

void ticketBookingSystem::queryQuantity(const vector<token> &V) {
	// ID DATE FROM TO;
	train T = Train.query(V[0].second).second;
	int d = V[1].second.asdate().asint();
	int l = T.getStationID(V[2].second), r = T.getStationID(V[3].second);
	for (int i = 0; i < seatN; i++) {
		int cls = T.getClassID(seatType[i]);
		if (cls != -1) {
			float price = 0;
			short _min = T.s[l].num[d][cls];
			for (int j = l+1; j <= r; j++) {
				price += T.s[j].price[cls];
				_min = std::min(_min, T.s[j].num[d][cls]);
			}
			os << price << ' ' << _min + station::INITIAL_QUANTITY << ' ';
		}
		else
			os << "-1 -1 ";
	}
...
void ticketBookingSystem::queryMyTicket(const vector<token> &V) {
	train &&T = Train.query(V[1].second).second;
	int id = V[0].second.asint();
	auto result = Log.findOrder(
		keyInfo(id, V[2].second.asdate(), T.catalog, T.ID, V[3].second, V[4].second)
	);
	if (!result.first) {
		os << "0\n";
		return;
