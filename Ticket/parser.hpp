...
//the exit procedure
	...
	else if (firsttoken == "modify_profile_without_password") {
		vector< std::pair<TYPE, String> > parameter;
		if (!scanner.hasMoreTokens()) REPORT;
		String tmp = scanner.nextToken();
		parameter.push_back(std::make_pair(_INT, tmp));
	...
	else if (firsttoken == "display") B.process(firsttoken, vector<token>());
	else if (firsttoken == "login_with_info") {
		vector< std::pair<TYPE, String> > parameter;
	...
	else if(firsttoken == "query_available_ticket"){
		// ID FROM TO DATE
		vector< std::pair<TYPE, String> > parameter;
		if (!scanner.hasMoreTokens()) REPORT;
		String tmp = scanner.nextToken();
	...
	else if (firsttoken == "query_my_ticket") {
	// ID FROM TO DATE
		vector< std::pair<TYPE, String> > parameter;
		if (!scanner.hasMoreTokens()) REPORT;
		String tmp = scanner.nextToken();
